		...
		{(pageType == "deck" || pageType == "fence") && 
			...
			<div className="card">
				...
				<table className="base-table text-center">
					...
					return <tr className="product-item" key={`${pageType}-product-item-${productItem.id}`}>
						<td>{productIndex+1}</td>
						<td>{productItem.name}</td>
						<td className="text-center">
							<button className="edit-btn d-inline-block" onClick={()=>{router.push(`/admin/deck/${productItem.id}`)}}>수정</button>
						</td>
					</tr>
				})}
				</table>
