...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	let apiUrl = `${process.env.SERVER_HOST_URL}/api/admin/deck/${queryData.productId}`;
...
export default function ProductIndex(props:any) {
    const {
		query: { productId: productId, pageType: pageType },
	} = useRouter();
	...
	return <section className="admin-section">
        ...
        <div className="card">
			<div className="card-body">
                ...
                <div className="product-title mb-4">상품 이미지
                {/* <i
				...
				{productData.images.products.map((imageItem:any, imageIndex:number)=>{
					return <div className="banner-item" key={`image-item-${imageIndex}-${imageIndex}`}>
				...
				<>
				<div className="product-title mb-4">3D 이미지
                {/* <i
							className="fa-solid fa-circle-plus ml-1 cursor-pointer"
							onClick={() => {
                                let newItem = {...pro}
				...
				{productData.images.models.map((imageItem:any, imageIndex:number)=>{
					return <div className="banner-item" key={`image-item-${imageIndex}-${imageIndex}`}>
								<div className="">이미지 {imageIndex+1}</div>
								<div
								    className="thumbnail"
                ...
                <div className="product-title mb-4">도안 이미지
                {/* <i
							className="fa-solid fa-circle-plus ml-1 cursor-pointer"
							onClick={() => {
                                let newItem = {...pro}
				...
				{productData.images.drafts.map((imageItem:any, imageIndex:number)=>{
					return <div className="banner-item" key={`image-item-${imageIndex}-${imageIndex}`}>
								<div className="">이미지 {imageIndex+1}</div>
								<div
								    className="thumbnail"
		...
		{pageType == "deck" && <div className="card">
			<div className="card-body">
                <div className="section-sub-title">상품 정보 관리
                {/* <i
							className="fa-solid fa-circle-plus ml-1 cursor-pointer"
							onClick={() => {
								// setPriceData([
					...
					{productData.productInfos.map((infoItem: any, infoIndex: number) => {
						return (
